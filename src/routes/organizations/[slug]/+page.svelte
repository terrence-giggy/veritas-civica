<script lang="ts">
	...
	const renderedContent = $derived(renderMarkdown(data.organization.body));
</script>
...
<nav class="max-w-content mx-auto px-6 pt-8">
	<div class="flex items-center gap-2 text-sm" style="color: hsl(var(--text-tertiary))">
		...
		<span>/</span>
		<a href="{base}/organizations" class="hover:text-primary transition-colors">Organizations</a>
		<span>/</span>
...
<Article
	...
	subtitle={data.organization.role ? data.organization.role : `From ${data.organization.source}`}
...
>
	...
	<AttributeDisplay parsedAttributes={data.organization.parsedAttributes} class="mb-8" />
...
<div class="max-w-content mx-auto px-6 pb-12">
	<div class="flex gap-4">
		<Button variant="outline" href="{base}/organizations">← All Organizations</Button>
